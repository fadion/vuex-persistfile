...
class VuexPersist {
  ...
  constructor(options) {
    ...
    this.options.pathAndFile = path.join(this.options.path, this.options.file)
  }
  ...
  loadState(store) {
    try {
      let data = this.driver.read(this.options.pathAndFile)
   ...
   */
  initialize(store) {
    if (this.driver.exists(this.options.pathAndFile)) {
