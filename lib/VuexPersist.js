...
class VuexPersist {
  ...
  constructor(options) {
    ...
    if(this.options.dailyBackup) {
      const date=new Date();
      const d = date.getDate();
      const m = date.getMonth() + 1;
      const y = date.getFullYear();
      this.options.backupPath = path.join(this.options.path, `${'' + y + (m<=9 ? '0' + m : m) + (d <= 9 ? '0' + d : d)}-${this.options.file}`);
    }
    if(this.options.hourlyBackup) {
      const date=new Date();
      const d = date.getDate();
      const m = date.getMonth() + 1;
      const y = date.getFullYear();
      const hour = date.getHours();
      this.options.backupPath = path.join(this.options.path,
        `${'' + y + (m<=9 ? '0' + m : m) + (d <= 9 ? '0' + d : d)}${''+ (hour<=9 ? '0' + hour : hour) +"00"}-${this.options.file}`);
    }
